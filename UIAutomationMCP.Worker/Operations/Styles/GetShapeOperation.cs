...
{
    public class GetShapeOperation : IUIAutomationOperation
    {
        private readonly ElementFinderService _elementFinderService;
        private readonly IOptions<UIAutomationOptions> _options;

        public GetShapeOperation(ElementFinderService elementFinderService, IOptions<UIAutomationOptions> options)
        {
        ...
        {
            var typedRequest = request.GetTypedRequest<GetShapeRequest>(_options);
            if (typedRequest == null)
            {
                return Task.FromResult(new OperationResult<StyleInfoResult> 
                { 
                    Success = false, 
                    Error = "Invalid request format. Expected GetShapeRequest.",
            ...
            try
            {
                ...
                var shape = stylesPattern.Current.Shape;
            ...
            catch (Exception ex)
            {
                ...
                { 
                    Success = false, 
                    Error = $"Failed to get shape: {ex.Message}",
                    Data = new StyleInfoResult { IsPatternAvailable = false }
                });
            }
        }
