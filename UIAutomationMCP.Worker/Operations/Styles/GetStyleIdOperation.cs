...
{
    public class GetStyleIdOperation : IUIAutomationOperation
    {
        private readonly ElementFinderService _elementFinderService;
        private readonly IOptions<UIAutomationOptions> _options;

        public GetStyleIdOperation(ElementFinderService elementFinderService, IOptions<UIAutomationOptions> options)
        {
        ...
        {
            var typedRequest = request.GetTypedRequest<GetStyleIdRequest>(_options);
            if (typedRequest == null)
            {
                return Task.FromResult(new OperationResult<StyleInfoResult> 
                { 
                    Success = false, 
                    Error = "Invalid request format. Expected GetStyleIdRequest.",
            ...
            try
            {
                ...
                var styleId = stylesPattern.Current.StyleId;
            ...
            catch (Exception ex)
            {
                ...
                { 
                    Success = false, 
                    Error = $"Failed to get style ID: {ex.Message}",
                    Data = new StyleInfoResult { IsPatternAvailable = false }
                });
            }
        }
