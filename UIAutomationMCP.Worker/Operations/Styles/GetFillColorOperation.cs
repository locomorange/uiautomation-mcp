...
{
    public class GetFillColorOperation : IUIAutomationOperation
    {
        private readonly ElementFinderService _elementFinderService;
        private readonly IOptions<UIAutomationOptions> _options;

        public GetFillColorOperation(ElementFinderService elementFinderService, IOptions<UIAutomationOptions> options)
        {
        ...
        {
            var typedRequest = request.GetTypedRequest<GetFillColorRequest>(_options);
            if (typedRequest == null)
            {
                return Task.FromResult(new OperationResult<StyleInfoResult> 
                { 
                    Success = false, 
                    Error = "Invalid request format. Expected GetFillColorRequest.",
            ...
            try
            {
                ...
                var fillColor = stylesPattern.Current.FillColor;
            ...
            catch (Exception ex)
            {
                ...
                { 
                    Success = false, 
                    Error = $"Failed to get fill color: {ex.Message}",
                    Data = new StyleInfoResult { IsPatternAvailable = false }
                });
            }
        }
