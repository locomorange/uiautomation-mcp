...
{
    public class GetExtendedPropertiesOperation : IUIAutomationOperation
    {
        private readonly ElementFinderService _elementFinderService;
        private readonly IOptions<UIAutomationOptions> _options;

        public GetExtendedPropertiesOperation(ElementFinderService elementFinderService, IOptions<UIAutomationOptions> options)
        {
        ...
        {
            var typedRequest = request.GetTypedRequest<GetExtendedPropertiesRequest>(_options);
            if (typedRequest == null)
            {
                return Task.FromResult(new OperationResult<StyleInfoResult> 
                { 
                    Success = false, 
                    Error = "Invalid request format. Expected GetExtendedPropertiesRequest.",
            ...
            try
            {
                ...
                var extendedProperties = stylesPattern.Current.ExtendedProperties;
            ...
            catch (Exception ex)
            {
                ...
                { 
                    Success = false, 
                    Error = $"Failed to get extended properties: {ex.Message}",
                    Data = new StyleInfoResult { IsPatternAvailable = false }
                });
            }
        }
