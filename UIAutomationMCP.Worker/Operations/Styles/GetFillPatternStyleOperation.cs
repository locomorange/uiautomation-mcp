...
{
    public class GetFillPatternStyleOperation : IUIAutomationOperation
    {
        private readonly ElementFinderService _elementFinderService;
        private readonly IOptions<UIAutomationOptions> _options;

        public GetFillPatternStyleOperation(ElementFinderService elementFinderService, IOptions<UIAutomationOptions> options)
        {
        ...
        {
            var typedRequest = request.GetTypedRequest<GetFillPatternStyleRequest>(_options);
            if (typedRequest == null)
            {
                return Task.FromResult(new OperationResult<StyleInfoResult> 
                { 
                    Success = false, 
                    Error = "Invalid request format. Expected GetFillPatternStyleRequest.",
            ...
            try
            {
                ...
                var fillPatternStyle = stylesPattern.Current.FillPatternStyle;
            ...
            catch (Exception ex)
            {
                ...
                { 
                    Success = false, 
                    Error = $"Failed to get fill pattern style: {ex.Message}",
                    Data = new StyleInfoResult { IsPatternAvailable = false }
                });
            }
        }
