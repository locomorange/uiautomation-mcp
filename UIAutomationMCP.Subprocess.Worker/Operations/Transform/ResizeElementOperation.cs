...
namespace UIAutomationMCP.Subprocess.Worker.Operations.Transform
{
    public class ResizeElementOperation : BaseUIAutomationOperation<ResizeElementRequest, TransformActionResult>
    {
        ...
        protected override UIAutomationMCP.Core.Validation.ValidationResult ValidateRequest(ResizeElementRequest request)
        {
            ...
            if (request.Width <= 0)
            {
                return UIAutomationMCP.Core.Validation.ValidationResult.Failure("Width must be greater than 0");
            }

            if (request.Height <= 0)
            {
                return UIAutomationMCP.Core.Validation.ValidationResult.Failure("Height must be greater than 0");
            }
