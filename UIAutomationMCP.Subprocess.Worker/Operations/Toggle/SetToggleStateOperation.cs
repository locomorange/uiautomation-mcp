...
namespace UIAutomationMCP.Subprocess.Worker.Operations.Toggle
{
    public class SetToggleStateOperation : BaseUIAutomationOperation<SetToggleStateRequest, ToggleActionResult>
    {
        ...
        protected override UIAutomationMCP.Core.Validation.ValidationResult ValidateRequest(SetToggleStateRequest request)
        {
            ...
            {
                return UIAutomationMCP.Core.Validation.ValidationResult.Failure("State is required");
            }

            if (!Enum.TryParse<ToggleState>(request.State, true, out _))
            {
                return UIAutomationMCP.Core.Validation.ValidationResult.Failure($"Invalid toggle state: {request.State}. Valid values: On, Off, Indeterminate");
            }
