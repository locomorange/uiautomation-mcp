...
namespace UIAutomationMCP.Worker.Operations.ControlTypeInfo
{
    public class FindElementsByControlTypeOperation : IUIAutomationOperation
    {
        ...
        public Task<OperationResult<ControlTypeSearchResult>> ExecuteAsync(string parametersJson)
        {
            var result = ExecuteInternalAsync(parametersJson);
            return result;
        }
        ...
        private Task<OperationResult<ControlTypeSearchResult>> ExecuteInternalAsync(string parametersJson)
        {
            try
            {
                var typedRequest = JsonSerializationHelper.Deserialize<FindElementsByControlTypeRequest>(parametersJson)!;
