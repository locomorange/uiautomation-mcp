...
namespace UIAutomationMCP.Worker.Operations.ControlTypeInfo
{
    public class GetControlTypeInfoOperation : IUIAutomationOperation
    {
        ...
        public Task<OperationResult<ControlTypeInfoResult>> ExecuteAsync(string parametersJson)
        {
            try
            {
                var typedRequest = JsonSerializationHelper.Deserialize<GetControlTypeInfoRequest>(parametersJson)!;
                var elementId = typedRequest.ElementId ?? "";
                var windowTitle = typedRequest.WindowTitle ?? "";
                var processId = typedRequest.ProcessId ?? 0;
                var validatePatterns = typedRequest.ValidatePatterns;
                var includeDefaultProperties = typedRequest.IncludeDefaultProperties;
